...
void sem_n_fun_dec(n_fun_dec *fun_dec) {
    ...
    entree_fonction();
    int ligne_entree = ligne;
    ajoute_ligne(entree, 0, 0, fun_dec->nom);
    ajoute_variable(fun_dec->nom, fun_dec->type);

    sem_n_l_dec(fun_dec->param);
    int nb_param = adresse_locale_courante - 1; // -1 pour la valeur de retour
    sem_n_type(fun_dec->type);
    sem_n_l_dec(fun_dec->variables);
    sem_n_instr(fun_dec->corps);

    /* nb de variables locales */
    code[ligne_entree].arg1 = adresse_locale_courante - nb_param - 1;

    sortie_fonction();
    ajoute_ligne(sortie, adresse_locale_courante - 1, 0, fun_dec->nom);
